...
export default function Resume(props) {
    ...
    function init() {
        ...
        for(let i = 0; i < N; i++) {
            for(let j = 0; j < i; j++) {
                ...
                // if(dist < effectController.minDistance && connections < effectController.maxConnections) {
                    adjMatrix[i][j] = 1;
                    adjMatrix[j][i] = 1;
                    // connections += 1;
                // }
    ...
    function animate() {
        ...
        for ( let i = 0; i < particleCount; i ++ ) {
            ...
            for (let j = 0; j < i; j++) {
                if(adjMatrix[i][j] === 1) {
                    positions[i] = particlePositions[i * 3];
                    positions[i + 1] = particlePositions[i * 3 + 1];
                    positions[i + 2] = particlePositions[i * 3 + 2];

                    positions[j] = particlePositions[j * 3];
                    positions[j + 1] = particlePositions[j * 3 + 1];
                    positions[j + 2] = particlePositions[j * 3 + 2];

                    colors[colorpos++] = fadeIn;
                    colors[colorpos++] = fadeIn;
                    colors[colorpos++] = fadeIn;
